...
- [~] Migraciones 001–046 en Supabase (proyecto "GDC", vía `gdc/supabase/migrations/` + `supabase db push`) — 024 a 030 corresponden a la ficha OT-02, 031 a 036 a la ficha OT-03 (ver más abajo). Las 001–036 están aplicadas y verificadas; las **037 a 046 están escritas pero PENDIENTES de aplicar** (`20260823110001_expedientes_update_juez_asistente`, `20260823120001_dias_no_habiles`, `20260823130001_plazo_contestacion`, `20260823140001_plazos_ejecutivos`, `20260823150001_audiencia_especial`, `20260823160001_estado_proceso`, `20260823170001_plazo_edicto_sucesiones`, `20260823180001_evento_embargo_decretado`, `20260823190001_monto_embargo_editable`, `20260823200001_anular_audiencia`) — ver las últimas entradas de este checklist
...
- [~] Audiencia especial (migración 041): **verificado en la transcripción del Código** a pedido del usuario. El **Art. 262 "Audiencia especial"** (Libro Segundo) enumera 8 supuestos que se deciden en audiencia especial cuando se practica una medida cautelar —exclusión de bienes del secuestro, rescisión del secuestro, levantamiento, separación del depositario, incidencias de la medida cautelar, reclamaciones al hacer el depósito en caso de embargo, rescisión de embargo o desembargo en procesos ejecutivos, y acumulación de procesos— y cierra con una cláusula abierta: también se sustancia en audiencia especial "toda petición, incidente y excepción de previo y especial pronunciamiento, y las excepciones en los procesos ejecutivos que requieran pronunciamiento judicial inmediato, así como toda cuestión que no pueda resolverse o no haya sido decidida en la audiencia preliminar". El **Art. 263** regula su tramitación. Consecuencia de diseño: **no tiene ventana parametrizable ni depende de una fase**, se convoca por el incidente, así que no entra por `avanzarFase` y no lleva fecha mínima ni límite; lleva un `motivo` obligatorio (columna nueva en `audiencias`), que es lo que distingue una especial de otra. Se agregó `especial` al enum `tipo_audiencia` y se muestra en el Calendario con su motivo. **Hallazgo colateral, no corregido:** el Código nunca usa "audiencia de fondo" — dice **"audiencia final"** (21 menciones contra 0), mientras el sistema usa `fondo` en el enum y en toda la UI. Es terminología, no lógica, y coincide con el vocabulario que usa el usuario; se deja anotado por si algún día conviene alinearlo
- [~] Desistimiento, conciliación y retiro como estados (migración 042): decisión del usuario — son **estados**, no tipos de documento. Nueva columna `expedientes.estado_proceso` (`en_tramite`/`desistido`/`conciliado`/`retirado`) con el mismo patrón que `estado_matrimonio` de OT-03: los tres estados distintos de `en_tramite` son terminales y cierran el expediente vía trigger, sin documento de cierre y sin motivo obligatorio (el estado es la justificación). Se amplió el `check` de `tipo_cierre` con `desistimiento`/`conciliacion`/`retiro` — localizando la restricción por su definición en `pg_constraint` en vez de adivinar el nombre generado, porque un drop fallido habría dejado el check viejo vivo y los valores nuevos habrían fallado al insertarse. Nuevo tipo de evento `estado_proceso_actualizado`, registrable desde la pestaña "Eventos y trazabilidad". **La mediación no entra aquí**: ya cierra por su documento "Acuerdo de Mediación" (OT-02/OT-03) y duplicarla daría dos formas distintas de cerrar por lo mismo. La lista blanca de columnas editables de la migración 037 se recreó incluyendo `estado_proceso`, para que el Juez pueda registrar un desistimiento
- [~] Reprogramar audiencias (migración 041, sin cambio de esquema adicional): el Calendario solo permitía cambiar el **estado** de una audiencia (celebrada/suspendida/continuada/terminada), no su fecha, así que un aplazamiento no tenía cómo registrarse. Nueva acción `reprogramarAudiencia` (solo Administrador, igual que el resto de la escritura sobre `audiencias`) desde la tarjeta "Audiencias" del detalle del expediente, con la fecha anterior y la nueva en auditoría, y aviso —no bloqueo— si la fecha nueva sale de la ventana legal
- [~] Plazo del edicto de sucesiones configurable (migración 043): aclarado por el usuario que el mes es el que tiene el interesado para **publicar el aviso del edicto emplazatorio en un diario de circulación nacional**, en **meses calendario** desde la emisión. Antes eran 30 días fijos escritos en el código, que no es lo mismo (febrero son 28, agosto 31) y que nadie podía ajustar. Nueva `configuracion_sistema.plazo_publicacion_edicto_meses` (default 1), editable en Administración, y `lib/plazo-edicto.ts` con la suma de meses respetando el fin de mes (un edicto del 31 de enero vence el 28 de febrero, no el 3 de marzo). Nuevo tipo de evento `reitero_publicacion_edicto`: se modeló como evento y no como estado porque puede repetirse y lo que interesa es el rastro de cada reitero — la tarjeta del Panel del Juez ahora muestra la fecha en que vencía el plazo y cuántos reiteros lleva. Verificado con 12 comprobaciones sobre las funciones reales (fin de mes, año bisiesto, cruce de año, el día exacto del límite)
- [ ] Informe automático por correo (día 15 al último día del mes siguiente): **en espera por decisión del usuario**, se configura al final. Es el único requisito de su transcripción que necesita infraestructura nueva (proveedor de correo transaccional y tareas programadas), no solo código
- [~] Confirmación de embargo atendido (migración 044): pedido del usuario tras revisar la alerta de ejecutivos. Se aclaró de paso que **el aviso ya salía el día 9** hábil (la condición era "más de 8 hábiles transcurridos", y el primer día que la cumple es el noveno) — no hizo falta cambiar el cálculo, solo decirlo explícitamente en el texto de la tarjeta, que no lo mencionaba. Lo que se agregó es la confirmación: nuevo tipo de evento `embargo_decretado` y un botón «Ya se decretó» en la propia alerta del Panel del Juez, que lo registra en un clic y saca al expediente de la lista. Se modeló como evento y no como columna de estado porque el hecho tiene fecha y autor (que es lo auditable), y porque al ser evento cuenta como movimiento del expediente para el contador de inactividad — decretar un embargo es trabajo real y no debería seguir sumando días de "sin trabajar". El mismo evento se puede registrar desde la pestaña "Eventos y trazabilidad" dejando en el detalle qué resolución lo decretó. La alerta excluye a los expedientes con ese evento **además** de los que ya avanzaron a la fase "Cumplimiento de embargo", porque el avance de fase es un acto aparte que puede ocurrir después
- [~] Coherencia del calendario y las alarmas (migración 046): a partir de la pregunta del usuario sobre si las alarmas se crean y se borran solas. Se aclaró primero la distinción: las alertas del Panel del Juez y la campana de notificaciones **se calculan al vuelo en cada carga**, no se guardan en ninguna tabla, así que aparecen y desaparecen solas al corregir el dato — no hay nada que pueda quedar colgado. El calendario es distinto porque ahí sí hay registros guardados (`audiencias`), y tenía tres huecos, los tres cerrados:
  - **La ventana guardada no se recalculaba.** `fecha_minima_calculada` y `fecha_limite_calculada` se escribían una sola vez al crear la audiencia, así que al corregir la fecha de notificación de la demanda quedaban congeladas: el detalle recalculaba al vuelo y la audiencia guardada seguía comparándose contra el rango viejo — dos números distintos para la misma audiencia. Era una inconsistencia **introducida al hacer editable esa fecha** en esta misma sesión. Ahora `actualizarDatosGenerales` recalcula la ventana de las audiencias preliminares del expediente (solo las preliminares: la de fondo se ancla en la preliminar y la especial no tiene ventana). El recálculo usa el cliente administrador a propósito y **solo toca esas dos columnas derivadas**: la escritura sobre `audiencias` está reservada al Administrador, pero la fecha de notificación la puede corregir el Juez o el Asistente asignado, y sin ese bypass el recálculo se descartaría en silencio para ellos. La alternativa —abrir `audiencias` a escritura de no-administradores— les daría además la capacidad de reprogramar, que es un permiso mucho mayor.
  - **No se podía borrar una audiencia.** Se agregó `anulada` al enum `estado_audiencia` más la columna `motivo_anulacion`, con la acción de anular (solo Administrador, motivo obligatorio, con auditoría). Se modeló como anulación y no como borrado físico: en un expediente judicial interesa saber que se señaló una audiencia y se anuló con su motivo, no que desapareció sin rastro. Las anuladas dejan de aparecer en el calendario y dejan de contar como audiencia programada para el cálculo de ventanas.
  - **El calendario no filtraba nada.** Las audiencias de expedientes ya cerrados seguían apareciendo como si fueran a celebrarse; ahora se marcan con el prefijo `[Cerrado]` en vez de ocultarse, porque el histórico importa pero no debe leerse como agenda.

  **Aclaración importante que quedó registrada:** no existe ninguna alarma que avise **fuera** de la aplicación — ni correo, ni notificación al teléfono, ni recordatorio, ni integración con Google Calendar. Todo se ve únicamente al entrar a GDC y abrir la pantalla. Eso es justo lo que resolvería el informe automático por correo que el usuario dejó en espera.

---

## 9. Carga de datos reales desde el Google Sheet / Excel del despacho — EN PAUSA

Decisión del usuario (2026-08-23): la fase de datos reales **queda pendiente hasta tener todo lo demás listo**. Lo que sigue son las decisiones ya confirmadas, para no perderlas.

- **Papel del Sheet:** carga inicial y uso en paralelo unas semanas, **sin sincronización**. No hace falta ninguna credencial de Google: con exportar a CSV alcanza. Se descartó publicar el Sheet en la web (dejaría los expedientes accesibles a cualquiera con la URL) y se descartó la sincronización continua (dos sistemas de verdad a la vez, y el Sheet no tiene dónde guardar eventos, cierre real, fases ni saldos).
- **Volumen real corregido: 126 expedientes, no 940.** El análisis previo en `comparativo_gdc_vs_excel.md` decía "184 filas con datos" por pestaña, contando filas donde *alguna* celda tiene algo; la columna "Físico o Electrónico" viene pre-rellenada en las 184 filas de cada pestaña aunque el resto esté vacío. Contando por número de expediente: **Ordinarios 3** (confirmado por el usuario: son los 3 procesos en marcha), **Sumarios 10**, **Ejecutivos 58**, **Sucesiones 47**, **Matrimonios 8**, Secuestro 0.
- **Fecha del semáforo:** la fecha de presentación del Sheet. Está **completa donde existe** (los 10 sumarios, los 58 ejecutivos y los 47 de sucesiones la tienen llena, sin huecos). Pero **Ordinarios y Matrimonios no tienen columna de fecha de presentación**: son 11 expedientes a resolver a mano.
- **Monto de mandamiento de pago → `monto_embargo_decretado`** (no a `cuantia`): es la base del embargo, confirmado por el usuario, y puede ampliarse después según el **Art. 744** (ver la entrada de la migración 045).
- **Tipo de sucesión: son DOS tipos** (Testada e Intestada), no tres. La "mixta" no es una categoría sino el caso de un proceso con bienes testados e intestados a la vez, así que se modelará como **marca aparte del expediente** y no como tercer valor del catálogo — decisión del usuario, y así el catálogo no inventa una categoría que la ley no nombra. Los valores reales en el Excel confirman que hoy solo se usan INTESTADA (44) y TESTADA (3); la mixta no se registra en ninguna parte todavía. **Pendiente de construir junto con la carga.**
- **Errores de captura ya detectados en el Excel, que harían fallar la importación:** 5 filas de Sucesiones tienen "SEGUNDO MUNICIPAL" (el despacho) escrito en la columna "Tipo de sucesión"; y hay filas con tipo de proceso pero sin número de expediente (36 con tipo contra 3 con número en Ordinarios), que fallarían porque el número es obligatorio y único.
- **Orden de trabajo acordado:** aplicar las migraciones pendientes → cargar los subtipos de Ejecución y Sucesiones → **import de prueba de 10 filas revisado a mano** → carga completa pestaña por pestaña.
